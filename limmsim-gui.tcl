...
proc dump_options { } {
  global dump_bmps threads gridsize timesteps nbitstr minmatch afflevel affenhance mhc capacity p_b_rec_mutate
  global init_macrophages init_bcells init_tcells ag_peptides v_prolif_b v_prolif_t v_prolif_ag 
  global vdiff_ag vdiff_ic vdiff_ab vdiff_t vdiff_b vdiff_ma
  global tau_ag tau_ic tau_ab tau_t tau_t_eff tau_b tau_plasma tau_memory
  global p_ma_ag p_ma_ic p_b_ag p_b_t p_ma_t p_remove_ag p_ag_ab p_b_become_mem th_diffusion th_reaction 
  global p_t_eff_become_mem p_t_eff_become_naive p_ma_become_mem
  global prolif_cycles_t prolif_cycles_b ab_secrete ag_injection_schedule

  global options

  set options "dump_bmps $dump_bmps\nthreads $threads\ngridsize $gridsize\ntimesteps $timesteps\nnbitstr $nbitstr"
  set options "$options\nminmatch $minmatch\nafflevel $afflevel\naffenhance $affenhance\nmhc $mhc"
  set options "$options\ncapacity $capacity\ninit_macrophages $init_macrophages\ninit_bcells $init_bcells\ninit_tcells $init_tcells"
  set options "$options\nag_peptides $ag_peptides\nv_prolif_b $v_prolif_b\nv_prolif_t $v_prolif_t\nv_prolif_ag $v_prolif_ag"
  set options "$options\nvdiff_ag $vdiff_ag\nvdiff_ic $vdiff_ic\nvdiff_ab $vdiff_ab\nvdiff_t $vdiff_t"
  set options "$options\nvdiff_b $vdiff_b\nvdiff_ma $vdiff_ma\ntau_ag $tau_ag\ntau_ic $tau_ic"
  set options "$options\ntau_ab $tau_ab\ntau_t $tau_t\ntau_t_eff $tau_t_eff\ntau_b $tau_b"
  set options "$options\ntau_plasma $tau_plasma\ntau_memory $tau_memory\np_ma_ag $p_ma_ag\np_ma_ic $p_ma_ic"
  set options "$options\np_b_ag $p_b_ag\np_b_t $p_b_t\np_ma_t $p_ma_t\np_remove_ag $p_remove_ag\np_b_become_mem $p_b_become_mem\np_b_rec_mutate $p_b_rec_mutate"
  set options "$options\np_t_eff_become_mem $p_t_eff_become_mem\np_t_eff_become_naive $p_t_eff_become_naive\np_ma_become_mem $p_ma_become_mem"
  set options "$options\np_ag_ab $p_ag_ab\nth_diffusion $th_diffusion\nth_reaction $th_reaction\nprolif_cycles_t $prolif_cycles_t"
  set options "$options\nprolif_cycles_b $prolif_cycles_b\nab_secrete $ab_secrete\nag_injection_schedule $ag_injection_schedule"
}

proc o_chosen { combo tab } {
  if { $tab == "Simulation" } {
    show_tab .sim
  } elseif { $tab == "Repertoire" } {
    show_tab .repertoire
  } elseif { $tab == "Diffusion" } {
    show_tab .diff
  } elseif { $tab == "Proliferation" } {
    show_tab .prolif
  } elseif { $tab == "Cell death" } {
    show_tab .death
  } elseif { $tab == "Interactions" } {
    show_tab .inter
  } elseif { $tab == "Further options" } {
    show_tab .rest
  }
}

proc show_tab { tab } {
  pack forget .sim .repertoire .diff .prolif .death .inter .rest
  pack $tab
}

proc create_option { tab name value y  } { 
  global $name
  set $name $value
  label .$tab.l_$name -text $name -bg white -font optFont
  entry .$tab.$name -relief sunken -width 20 -textvariable $name 
  pack .$tab.l_$name -fill x
  pack .$tab.$name -fill x
}

proc start_stop {} {
  global running options lio step 

  if { $running } {
    set running 0 
    reset_data
    .start configure -text "Start"
    .sim_state configure -text "State: not running"
    flush $lio
    fileevent $lio readable 
    exec kill [pid $lio]
    catch { close $lio }
  } else {
    dump_options
    set lio [open "|./limmsim -i" r+]
    puts $lio $options
    puts $lio "." 
    flush $lio
    set running 1
    set step 0
    fileevent $lio readable display_data
    .start configure -text "Stop"
    .sim_state configure -text "State: running"
    foreach l { "b" "t" "ag" "ab" } { 
      .output.$l delete all ; .output.$l create text 10 10 -text "Initialisiation..." -anchor w -font titleFont
    }
 }
}

proc reset_data {} {
  global running pid io_state max_x scale_x max_b max_t
  set running 0
  set pid 0
  set io_state 0
  set max_x 400 ; set scale_x 1.0 ; 

  foreach l { "ag" "ab" "b" "t" } {
    global max_$l scale_$l
    set max_$l 300 ; set scale_$l 1.0
  }
  foreach l { "ag" "ab" "b_pres" "b_prolif" "b_plasma" "t_prolif" "t_armed" } {
    global pts_$l 
    set pts_$l [list]; lappend pts_$l 0 0; 
  }
}

proc plot_b { line } { 
  global max_b scale_x scale_b pts_b_pres pts_b_prolif pts_b_plasma st_step 
  set b_pres [lindex [split $line "\t"] 2]
  set b_prolif [lindex [split $line "\t"] 3]
  set b_plasma [lindex [split $line "\t"] 4]
  if { $b_pres > $max_b || $b_prolif > $max_b || $b_plasma > $max_b } { 
    set max_b [expr $max_b * 2] ; set scale_b [expr $scale_b / 2] ;
  }
  if { [expr $st_step % 3 == 0] } { 
    lappend pts_b_pres $st_step $b_pres
    lappend pts_b_prolif $st_step $b_prolif
    lappend pts_b_plasma $st_step $b_plasma
  }
  .output.b delete all 
  .output.b create line $pts_b_pres -fill #669900
  .output.b create line $pts_b_prolif -fill #1b6ca5
  .output.b create line $pts_b_plasma -fill #cc6600
  .output.b scale all 0 0 [expr 1/$scale_x] -$scale_b
  .output.b move all 0 300
  .output.b create text 10 10 -text "B-Cells (y_max=$max_b)" -anchor w -font titleFont
  .output.b create rect 10 25 25 40 -fill #669900
  .output.b create text 30 35 -text "Presenting antigen" -anchor w
  .output.b create rect 10 45 25 60 -fill #1b6ca5
  .output.b create text 30 55 -text "Proliferating" -anchor w
  .output.b create rect 10 65 25 80 -fill #cc6600
  .output.b create text 30 75 -text "Plasma" -anchor w
}

proc plot_t { line } { 
  global max_t scale_x scale_t pts_t_prolif pts_t_armed st_step

  set t_prolif [lindex [split $line "\t"] 1]
  set t_armed [lindex [split $line "\t"] 2]
  if { $t_prolif > $max_t || $t_armed > $max_t } { 
    set max_t [expr $max_t * 2] ; set scale_t [expr $scale_t / 2] ;
  }
  if { [expr $st_step % 3 == 0] } { 
    lappend pts_t_prolif $st_step $t_prolif
    lappend pts_t_armed $st_step $t_armed
  }
  .output.t delete all 
  .output.t create line $pts_t_prolif -fill #1b6ca5
  .output.t create line $pts_t_armed -fill #cc6600
  .output.t scale all 0 0 [expr 1/$scale_x] -$scale_t
  .output.t move all 0 300
  .output.t create text 10 10 -text "T-Cells (y_max=$max_t)" -anchor w -font titleFont
  .output.t create rect 10 25 25 40 -fill #1b6ca5
  .output.t create text 30 35 -text "Proliferating" -anchor w
  .output.t create rect 10 45 25 60 -fill #cc6600
  .output.t create text 30 55 -text "Armed" -anchor w
}

proc plot_points { c sc_x sc_y txt pts } {
  $c delete all
  $c create line $pts -fill #1b6ca5
  $c scale all 0 0 [expr 1/$sc_x] -$sc_y
  $c move all 0 300
  $c create text 10 10 -text $txt -anchor w -font titleFont
}

proc display_data {} {
  global lio io_state foto gridsize
  global st_step st_ag st_ab st_ic st_ma st_b st_t
  global pts_ag pts_ab pts_b_pres pts_b_prolif pts_b_plasma
  global max_x max_ag max_ab max_b
  global scale_x scale_ag scale_ab scale_b

  set status [catch { gets $lio line } result]
  if { $status != 0 } {
  } elseif { $result >= 0 } {
    if { [string match step* $line ] } {
      set io_state 1
      set st_step [string range $line 5 end ]
      if { $st_step > $max_x } { 
        set max_x [expr $max_x + 400]
        set scale_x [expr $scale_x + 1]
      }
    } elseif { $io_state == 1 } { 
      set io_state [expr $io_state+1] ; set st_t "$line\t "
      plot_t $line
      # B-Zellen
    } elseif { $io_state == 2 } {
      set io_state [expr $io_state+1] ; set st_b "$line\t "
      plot_b $line
      # Antigen
    } elseif { $io_state == 3 } {
      set io_state [expr $io_state+1] ; set st_ag $line
      if { $st_ag > $max_ag } {
        set max_ag [expr $max_ag * 2] ; set scale_ag [expr $scale_ag / 2] ;
      }
      if { [expr $st_step % 3 == 0] } { lappend pts_ag $st_step $st_ag }
      plot_points .output.ag  $scale_x $scale_ag "Antigen (y_max=$max_ag)" $pts_ag
      # Antikoerper
    } elseif { $io_state == 4 } {
      set io_state [expr $io_state+1] ; set st_ab $line
      if { $st_ab > $max_ab } {
        set max_ab [expr $max_ab * 2] ; set scale_ab [expr $scale_ab / 2] ;
      }
      if { [expr $st_step % 3 == 0] } { lappend pts_ab $st_step $st_ab }
      plot_points .output.ab  $scale_x $scale_ab "Antibody (y_max=$max_ab)" $pts_ab
      # Immunkomplexe
    } elseif { $io_state == 5 } {
      set io_state [expr $io_state+1] ; set st_ic $line
      # Makrophagen
    } elseif { $io_state == 6 } {
      set io_state [expr $io_state+1] ; set st_ma $line
    }
  } elseif { [eof $lio] } {
    exec kill [pid $lio]
    set running 0 
...
font create titleFont -family Helvetica -weight bold
font create optFont -family Helvetica -weight normal

frame .tabs

set optiontab "Simulation"
combobox .o_chooser -borderwidth 1 -textvariable optiontab -width 17 \
    -editable false -highlightthickness 1 -command { o_chosen }

.o_chooser list insert end "Simulation" 
.o_chooser list insert end "Repertoire" 
.o_chooser list insert end "Diffusion" 
.o_chooser list insert end "Proliferation" 
.o_chooser list insert end "Cell death" 
.o_chooser list insert end "Interactions" 
.o_chooser list insert end "Further options" 

button .start -text "Start" -font optFont -command { start_stop }

frame .output

canvas .output.b -width 400 -height 300
canvas .output.t -width 400 -height 300
canvas .output.ag -width 400 -height 300
canvas .output.ab -width 400 -height 300

grid .output.b -column 0 -row 0
grid .output.t -column 0 -row 1
grid .output.ag -column 1 -row 0
grid .output.ab -column 1 -row 1
...
label .stats.step.l -text "Time step" 
label .stats.ag.l -text "Ag"  
label .stats.b.l -text "B" 
label .stats.t.l -text "T" 
label .stats.ab.l -text "Ab"
label .stats.ic.l -text "IC"
label .stats.ma.l -text "Ma"

label .stats.step.v -text "0" -textvariable st_step
label .stats.ag.v -text "0" -textvariable st_ag
label .stats.b.v -text "0" -textvariable st_b
label .stats.t.v -text "0" -textvariable st_t 
label .stats.ab.v -text "0" -textvariable st_ab
label .stats.ic.v -text "0" -textvariable st_ic
label .stats.ma.v -text "0" -textvariable st_ma


pack .stats.step.l .stats.step.v -anchor w 
pack .stats.ag.l .stats.ag.v -anchor w 
pack .stats.ab.l .stats.ab.v -anchor w 
pack .stats.b.l .stats.b.v -anchor w 
pack .stats.t.l .stats.t.v -anchor w 
pack .stats.ic.l .stats.ic.v -anchor w 
pack .stats.ma.l .stats.ma.v -anchor w 

grid .stats.step -column 0 -row 0
grid .stats.ag  -column 1 -row 0
grid .stats.b  -column 2 -row 0
grid .stats.t -column 3 -row 0
grid .stats.ab  -column 4 -row 0
grid .stats.ic  -column 5 -row 0
grid .stats.ma  -column 6 -row 0

pack .stats -side bottom

pack .output -side right

pack [label .sim_title -text "Simulation" -borderwidth 2 -bg #cccccc ] -fill x
pack .start -pady 2m
pack [label .sim_state -text "State: not running" -font optFont] 
pack [label .o_title -text "Parameter" -borderwidth 2 -bg #cccccc ] -fill x -pady 2m
pack .o_chooser -anchor w

frame .sim 

set dump_bmps 0
label .sim.l_dump_bmps -bg white -text dump_bmps -font optFont
checkbutton .sim.dump_bmps -textvariable dump_bmps -relief sunken 
pack .sim.l_dump_bmps -fill x
pack .sim.dump_bmps -fill x

create_option "sim" "threads" "2" "1"
create_option "sim" "gridsize" "40" "2"
create_option "sim" "timesteps" "5000" "3"
create_option "sim" "capacity" "64" "4"
create_option "sim" "init_macrophages" "32" "5"
create_option "sim" "init_bcells" "12" "6"
create_option "sim" "init_tcells" "12" "7"

frame .repertoire

create_option "repertoire" "nbitstr" "16" "0"
create_option "repertoire" "afflevel" "0.05" "1"
create_option "repertoire" "minmatch" "14" "2"
create_option "repertoire" "affenhance" "0.77" "3"
create_option "repertoire" "ag_peptides" "3" "4"
create_option "repertoire" "mhc" "123" "5"
create_option "repertoire" "p_b_rec_mutate" "0.0" "6"

frame .diff

create_option "diff" "vdiff_ag" "0.1" "0"
create_option "diff" "vdiff_ic" "0.1" "1"
create_option "diff" "vdiff_ab" "0.5" "2"
create_option "diff" "vdiff_t" "0.02" "3"
create_option "diff" "vdiff_b" "0.02" "4"
create_option "diff" "vdiff_ma" "0.01" "5"
create_option "diff" "th_diffusion" "64" "6"

frame .prolif

create_option "prolif" "v_prolif_b" "0.05" "0"
create_option "prolif" "v_prolif_t" "0.025" "1"
create_option "prolif" "v_prolif_ag" "0.02" "2"
create_option "prolif" "prolif_cycles_t" "6" "3"
create_option "prolif" "prolif_cycles_b" "6" "4"

frame .death

create_option "death" "tau_ag" "10000" "0"
create_option "death" "tau_ic" "1000" "1"
create_option "death" "tau_ab" "200" "2"
create_option "death" "tau_t" "200" "3"
create_option "death" "tau_t_eff" "50" "4"
create_option "death" "tau_b" "200" "5"
create_option "death" "tau_plasma" "100" "6"
create_option "death" "tau_memory" "10000" "7"

frame .inter

create_option "inter" "p_ma_ag" "0.0005" "0"
create_option "inter" "p_ma_ic" "1" "1"
create_option "inter" "p_b_ag" "1" "2"
create_option "inter" "p_b_t" "3" "3"
create_option "inter" "p_ma_t" "0.8" "4"
create_option "inter" "p_remove_ag" "0.01" "5"
create_option "inter" "p_ag_ab" "0.5" "6"
create_option "inter" "p_b_become_mem" "0.05" "7"
create_option "inter" "p_t_eff_become_mem" "0.0" "8"
create_option "inter" "p_t_eff_become_naive" "0.0" "9"
create_option "inter" "p_ma_become_mem" "0.0" "10"
create_option "inter" "th_reaction" "64" "11"

frame .rest 

create_option "rest" "ab_secrete" "16" "0"
create_option "rest" "ag_injection_schedule" "100 2500" "1"
